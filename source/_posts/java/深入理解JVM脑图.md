...
![深入理解JVM脑图](http://wx1.sinaimg.cn/large/006b7Nxngy1g1g31rss95j30mj0afgnc.jpg)


## 1. 运行时数据区域
- Java虚拟机在执行Java程序的过程中会把它所管理的内存划分为若干个不同的数据区域。这些区域有各自的用途,以及创建和销毁的时间，有的区域随着虚拟机进程的启动而存在，有些区域则是依赖用户线程的启动和结束而建立和销毁。

![](http://wx1.sinaimg.cn/large/006b7Nxngy1g1g327ko68j30g20anab3.jpg)
...
![](http://wx1.sinaimg.cn/large/006b7Nxngy1g1g333ghndj30mx04x415.jpg)
...
![](http://wx1.sinaimg.cn/large/006b7Nxngy1g1g33qxpsmj30fu0c2tay.jpg)
...
- 类从被加载到虚拟机内存中开始，到卸载出内存为止，它的整个生命周期包括：加载（Loading）、验证（Verification）、准备(Preparation)、解析(Resolution)、初始化(Initialization)、使用(Using)和卸载(Unloading)7个阶段。其中准备、验证、解析3个部分统称为连接（Linking）
  ![](http://wx1.sinaimg.cn/large/006b7Nxngy1g1g36ub9ibj30hf06774b.jpg)
...
![对象的创建](http://wx1.sinaimg.cn/large/006b7Nxngy1g1g354jorrj30830el0sy.jpg)
...
![](http://wx1.sinaimg.cn/large/006b7Nxngy1g1g37jd8gwj30ih0900uu.jpg)

#### 直接指针定位
如果是直接指针访问，Java堆的布局就必须考虑如何放置访问类型数据相关。

![](http://wx1.sinaimg.cn/large/006b7Nxngy1g1g3817gxxj30kf09bwgj.jpg)
